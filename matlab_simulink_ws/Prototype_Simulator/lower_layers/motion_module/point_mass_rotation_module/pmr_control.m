function commands_bottom = pmr_control(t, states,xyz_ds, vxyz_d_fws,control_mode,sample_time_control)
...
%     speed_ds = sqrt(sum(vxyz_ds.^2,1));
    ...
    if bitand(control_mode,0b000001) == 0
        ...
        vxyz_ds = zeros(3,n);
        vxyz_ds(3,:) = (xyz_ds(3,:) - states(3,:))/T_p;
    else
        vxyz_ds = (xyz_ds - states(1:3,:))/T_p;
    end
