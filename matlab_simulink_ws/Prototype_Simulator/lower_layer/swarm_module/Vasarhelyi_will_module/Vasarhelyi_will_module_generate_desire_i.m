...
[r_com,v_flock,tau_will,r_rep0,p_rep,r_frict0,C_frict,v_frict,p_frict,a_frict,r_shill0,...
    v_shill,p_shill,a_shill,v_max,~,heights,~,posShill,velShill] =...
...
if ~isempty(dis_to_neighbor)
    ...
    v_rep_id = zeros(dimension,1);
    if ~isempty(inRepInCom)
        disijInRep = repmat(dis_to_neighbor(inRepInCom),dimension,1);
        v_rep_id = p_rep * sum((r_rep0 - disijInRep) .* posid_to_neighbor(:,inRepInCom)./disijInRep,2);
%         vRepId = [1;1];
    end
