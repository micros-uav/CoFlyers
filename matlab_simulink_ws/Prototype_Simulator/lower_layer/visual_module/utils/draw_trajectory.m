...
xs = squeeze(states_series(1,:,:))'; xs(end,:) = nan;
ys = squeeze(states_series(2,:,:))';
if dim == 3
    cs = squeeze(sum(sqrt(states_series(4:6,:,:).^2),1))';
    zs = squeeze(states_series(3,:,:))';
    trajectories = cat(3,xs,ys,zs,cs);
elseif dim == 2
    cs = squeeze(sum(sqrt(states_series(4:5,:,:).^2),1))';
    trajectories = cat(3,xs,ys,cs);
end

if size(trajectories,2) < 3
