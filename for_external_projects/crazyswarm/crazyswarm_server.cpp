...
public:
    ...
    , m_initializedPosition(false)
  {
    ...
    test_pub = n.advertise<geometry_msgs::PoseStamped>(m_tf_prefix + "/pose", 10);
    test_timer = n.createTimer(ros::Duration(1.0/30.0),&CrazyflieROS::callback_pub_pose,this);
  }
...
private:
  ...
  void callback_pub_pose(const ros::TimerEvent&){
    test_pub.publish(test_pose);
  }
public:
  void test_update_pose(float x,float y,float z, float qw, float qx, float qy, float qz){
...
public:
  ...
  void runFast()
  {
    ...
    //////////////////////////////////////////////////////////////////
    {
      ...
      for (auto cf : m_cfs) {
        // bool found = publishRigidBody(cf->frame(), cf->id(), states);
        cf->test_update_pose(states[i].x, states[i].y, states[i].z, states[i].qw, states[i].qx, states[i].qy, states[i].qz);
        i++;
      }
    }
